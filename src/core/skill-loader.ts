...
export async function listSkillFiles(skillsDir: string): Promise<string[]> {
  try {
    ...
    return allFiles.filter((filePath) => filePath.endsWith('skill.yaml')).sort();
  } catch (error) {
...
export async function loadSkillFile(filePath: string): Promise<LoadedSkill> {
  ...
  if (!validation.valid || validation.data === undefined) {
    throw new Error(`Invalid skill file ${filePath}: ${validation.errors.join('; ')}`);
  }
...
export async function loadAllSkills(skillsDir: string): Promise<LoadedSkill[]> {
  ...
  return Promise.all(skillFiles.map(async (filePath) => loadSkillFile(filePath)));
}

export async function loadSkillsFromDirectories(skillDirs: string[]): Promise<LoadedSkill[]> {
  const allEntries = await Promise.all(skillDirs.map(async (skillDir) => loadAllSkills(skillDir)));
